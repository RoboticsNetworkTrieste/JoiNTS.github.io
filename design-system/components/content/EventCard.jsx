...
export function EventCard({ day, month, title, where, when, kind, href, ctaLabel = 'Partecipa', style, className, ...rest }) {
  return (
    <Card pad="sm" className={className} style={style} {...rest}>
      <div className="joints-event">
        <div className="joints-event__date">
          <span className="joints-event__day">{day}</span>
          <span className="joints-event__month">{month}</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)', minWidth: 0 }}>
          {kind && <span className="joints-kicker">{kind}</span>}

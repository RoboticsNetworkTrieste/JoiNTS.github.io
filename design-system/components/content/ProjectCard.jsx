...
export function ProjectCard({ title, summary, state = 'live', stateLabel, tags = [], stack, contributors, updated, href, icon = 'circuit-board', style, className, ...rest }) {
  return (
    <Card href={href} interactive={!!href} className={className} style={{ minHeight: 0, ...style }} {...rest}>
      <div className="torc-project__head">
      ...
      <h3 className="torc-project__title" style={{ marginTop: 'var(--space-4)' }}>{title}</h3>
      <p className="torc-project__body">{summary}</p>
      {tags.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)' }}>
          {tags.map(t => <Badge key={t}>{t}</Badge>)}
        </div>
      )}
      <div className="torc-project__meta">
        {stack && <span className="torc-project__meta-item"><Icon name="cpu" size={13} />{stack}</span>}
        {contributors != null && <span className="torc-project__meta-item"><Icon name="users" size={13} />{contributors}</span>}
        {updated && <span className="torc-project__meta-item"><Icon name="git-branch" size={13} />{updated}</span>}
      </div>
    </Card>
  );
}
